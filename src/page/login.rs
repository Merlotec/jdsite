...
use actix_web::{body::Body, http, http::Cookie, web, HttpRequest, HttpResponse};
...
pub async fn login_get(data: web::Data<Arc<SharedData>>, req: HttpRequest) -> HttpResponse {
    match data.authenticate_context_from_request(&req, false) {
        Ok(ctx) => login_template(ctx, &data, String::new()),
        ...
        Ok(old_ctx) => {
            if !form.username.is_empty() && !form.password.is_empty() {
                match data.login(
                    ...
                    Err(login::AuthError::IncorrectPassword) => login_template(
                        old_ctx,
                        &data,
                        "Incorrect username and password combination".to_owned(),
                    ),
                    Err(login::AuthError::NoUser) => login_template(
                        old_ctx,
                        &data,
                        "Incorrect username and password combination".to_owned(),
                    ),
                    Err(login::AuthError::DbError(e)) => {
                ...
                login_template(
                    old_ctx,
                    &data,
                    "Please provide a username and password!".to_owned(),
                )
            }
        }
        Err(e) => HttpResponse::new(http::StatusCode::INTERNAL_SERVER_ERROR)
...
pub async fn change_password_get(
    data: web::Data<Arc<SharedData>>,
    req: HttpRequest,
    link_token_str: web::Path<String>,
) -> HttpResponse {
    match link::LinkToken::from_str(&link_token_str) {
        Ok(token) => match data.authenticate_context_from_request(&req, false) {
            Ok(ctx) => change_password_page(ctx, &data, token, ""),
            Err(e) => HttpResponse::new(http::StatusCode::INTERNAL_SERVER_ERROR)
                .set_body(Body::from(format!("Error: {}", e))),
        },
        Err(e) => HttpResponse::new(http::StatusCode::INTERNAL_SERVER_ERROR)
                            ...
                            if let link::Link::ChangePassword(user_id) = entry.link {
                                match data.user_db.fetch(&user_id) {
                                    Ok(Some(user)) => {
                                        ...
                                        match data.login_db.change_password(
                                            &login_key,
                                            &form.password,
                                            false,
                                        ) {
                                            Ok(_) => {
                                                // Destroy link
                                                let _ =
                                                    data.link_manager.db().remove_silent(&token);
            ...
            if let link::Link::CreateUser(user_agent) = entry.link {
                ...
                if let user::UserAgent::Organisation(org_id) = &user_agent {
                    if let Ok(Some(org)) = data.org_db.fetch(org_id) {
                        info = "Create an organisation administrator account for '".to_owned()
                            + &org.name
                            + "':"
                    }
                }
...
pub async fn create_account_get(
    data: web::Data<Arc<SharedData>>,
    req: HttpRequest,
    link_token_str: web::Path<String>,
) -> HttpResponse {
    match link::LinkToken::from_str(&link_token_str) {
        Ok(token) => match data.authenticate_context_from_request(&req, false) {
            Ok(ctx) => create_account_page(ctx, &data, token, ""),
            Err(e) => HttpResponse::new(http::StatusCode::INTERNAL_SERVER_ERROR)
                .set_body(Body::from(format!("Error: {}", e))),
        },
        Err(e) => HttpResponse::new(http::StatusCode::INTERNAL_SERVER_ERROR)
    ...
    match data.authenticate_context_from_request(&req, false) {
        Ok(ctx) => {
            if let Ok(Some(login_entry)) = data.login_db.db().fetch(&form.username) {
                if let Ok(link_token) = data.link_manager.create_link(
                    link::Link::ChangePassword(login_entry.user_id),
            ...
            let page = data
                .handlebars
                .render("login/reset_password_sent", &())
                .unwrap();
