...
pub async fn unreviewed_get(data: web::Data<Arc<SharedData>>, req: HttpRequest, org_path_str: web::Path<String>) -> HttpResponse {
                ...
                if ctx.user.user_agent.can_view_org(&org_id) {
                    match data.org_db.fetch(&org_id) {
                        Ok(Some(org)) => {
                            ...
                            let content = data.handlebars.render("sections/section_list", &json!({
                                "section_rows": rows,
                            })).unwrap();
                            ...
                            let nav = page::org_nav(&ctx, &data, org_id, &org, dir::org_path(org_id) + dir::UNREVIEWED_SECTIONS_PAGE);
                            ...
                            let body = page::render_page(Some(ctx), &data, dir::APP_NAME.to_owned() + " | " + &org.name + " - Unreviewed Sections", dir::APP_NAME.to_owned(), org_page).unwrap();
